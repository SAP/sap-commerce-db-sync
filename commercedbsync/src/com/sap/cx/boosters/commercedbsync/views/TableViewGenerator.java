...
public class TableViewGenerator {
	...
	/**
	 * Generates {@link ViewConfigurationContext} context with all required data for <code>VIEW</code> DDL generation.
	 * @param tableName raw table name which should be a source of data
	 * @param migrationContext holds all information about source data
	 * @return
	 * @throws Exception
	 */
	public ViewConfigurationContext generateForTable(final String tableName, final MigrationContext migrationContext)
		...
		try {
			...
			return new ViewConfigurationContext(tableName, tableVieName, columns, customColumns, whereView, viewColumnPrefix);
		} catch (Exception e) {
	...
	/**
	 * generates DDL VIEW definition based on ctx for given table. Template of that string is:<br/>
	 * <code>CREATE OR ALTER VIEW ${name} AS SELECT ${columns} FROM ${from}</code>
	 * 
	 * where:
	 * 
	 * <li>
	 * 	<ul>name - view name</ul>
	 *  <ul>columns - view columns with optional custom column definition</ul>
	 *  <ul>from - from source table, or optional custom DQL <code>FROM</code> section</ul> 
	 * </li>
	 * @param ctx
	 * @return
	 */
	...
	/**
	 * method generates from context list of columns, where if there is custom column definition it will replace original one
	 * @param ctx
	...
	public String generateColumnList(final ViewConfigurationContext ctx) {
		...
		{
			newColumnSet = newColumnSet.stream().map(column -> viewColumnPrefix + "." + column).collect(Collectors.toSet());
		}
